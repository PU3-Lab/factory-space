...
발표와 포트폴리오에서는 단순히 “질문하면 답한다”가 아니라, `operator_guide`가 RAG, 현재 게임 상태, guardrail을 어떻게 함께 사용하는지 보여주는 것이 중요하다.
...
복합 질문은 한 문장 안에 여러 의도가 들어 있는 상황을 보여준다.
...
기대 흐름:
-> 질문을 장비 설명과 레시피 질문으로 분리
-> 각 질문에 맞는 RAG 문서 검색
-> LLM이 튜토리얼 NPC 톤으로 단계적인 답변 생성
```

이 시연은 `operator_guide`가 단순 키워드 매칭이 아니라, 질문 의도를 나누고 근거를 합쳐 답변한다는 점을 보여준다.

### 19.2 현재 상태 기반 문제 해결 시연

현재 상태 기반 문제 해결은 RAG 문서만으로 답하기 어려운 상황을 보여준다.
...
기대 흐름:
-> 현재 상태가 필요한 질문인지 판단
-> current_game_state 참고
-> RAG 문제 해결 문서와 현재 상태를 함께 사용
-> 원인, 확인 순서, recommended_actions 제공
```

이 시연은 `operator_guide`가 일반 매뉴얼 답변을 넘어, 게임 상태를 반영한 진단형 에이전트로 동작한다는 점을 보여준다.

### 19.3 프롬프트 인젝션 방어 시연

프롬프트 인젝션 방어는 플레이어 입력이 시스템 지시를 덮어쓰지 못한다는 점을 보여준다.
...
기대 흐름:
-> 플레이어 입력을 명령이 아니라 데이터로 취급
-> 시스템 프롬프트와 내부 정책 공개 거부
-> 공장 매뉴얼 범위의 질문으로 다시 유도
```

이 시연은 RAG 품질뿐 아니라 운영 안정성, 보안적 사고, guardrail 설계를 함께 보여주는 포트폴리오 포인트다.

### 19.4 시연 화면에서 보여줄 metadata

시연 화면에서는 최종 답변뿐 아니라 아래 metadata를 함께 보여주는 것이 좋다.
...
- retrieval
- selectedAgent
- selectedLeafAgent 또는 question_type
- requiresCurrentGameState
- usedCurrentGameState
- recommended_actions
```

이 정보는 “LLM이 그냥 말한 답변”이 아니라 “검색 근거와 상태 판단을 거친 답변”이라는 점을 설명하는 근거가 된다.
...
- 2026-06-16: 발표/포트폴리오용 3단계 시연 흐름을 추가했다.

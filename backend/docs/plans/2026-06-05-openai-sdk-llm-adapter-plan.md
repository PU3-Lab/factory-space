...
**Scope:** `backend/src/llm`의 OpenAI 호출 방식과 관련 테스트, 의존성, 짧은 문서 설명만 변경한다. 모델명 변경, Responses API 전환, 프롬프트 변경, local provider 재설계는 이번 범위에서 제외한다.
...
- [ ] `openai` dependency가 backend runtime dependency에 추가되었다.
- [ ] `OpenAILLMAdapter`가 공식 SDK client를 통해 `chat.completions.create()`를 호출한다.
- [ ] `OpenAILLMAdapter`는 slot `api_key`가 없으면 provider 호출 없이 `None`을 반환한다.
- [ ] `OpenAILLMAdapter`는 slot `model`이 없으면 provider 호출 없이 `None`을 반환한다.
- [ ] `OpenAILLMAdapter`는 slot `base_url`이 있으면 SDK client 생성에 반영한다.
- [ ] SDK 예외나 비정상 응답은 예외 전파가 아니라 `None` 반환으로 변환한다.
- [ ] response text는 `choices[0].message.content`에서 추출한다.
- [ ] 빈 문자열 또는 whitespace-only content는 `None`으로 변환한다.
- [ ] JSON object 문자열은 기존처럼 trim하지 않고 그대로 반환한다.
- [ ] `LocalLLMAdapter`는 기존 OpenAI-compatible HTTP 호출 계약을 유지한다.
- [ ] 실제 OpenAI API를 호출하지 않는 unit test로 검증한다.
...
- [ ] `backend/pyproject.toml`의 runtime dependencies에 `openai`를 추가한다.
- [ ] `cd backend && uv lock`으로 lock file을 갱신한다.
- [ ] `cd backend && uv run python -c "from openai import OpenAI"`로 import smoke를 확인한다.
...
- [ ] fake SDK client를 추가한다.
- [ ] `OpenAILLMAdapter.invoke()`가 `chat.completions.create()`에 `model`, `messages`, `max_tokens`, `temperature`를 전달하는 실패 테스트를 작성한다.
- [ ] SDK 응답의 `choices[0].message.content`를 반환하는 실패 테스트를 작성한다.
- [ ] api key가 없으면 SDK client를 호출하지 않는 테스트를 유지하거나 갱신한다.
- [ ] SDK 예외 발생 시 `None`을 반환하는 테스트를 작성한다.
- [ ] empty/whitespace content는 `None`을 반환하는 테스트를 유지한다.
- [ ] JSON object raw text를 그대로 반환하는 테스트를 유지한다.
- [ ] `LocalLLMAdapter` 테스트는 기존 HTTP fake 기반으로 남겨 local regression을 막는다.
...
- [ ] `from openai import OpenAI`를 파일 상단에 추가한다.
- [ ] OpenAI SDK client에 필요한 protocol을 추가한다.
- [ ] `OpenAILLMAdapter`에 테스트 주입용 `client` 또는 `client_factory` 필드를 추가한다.
- [ ] 실제 client 생성 시 `api_key`, optional `base_url`, timeout 설정을 반영한다.
- [ ] `invoke()`에서 SDK client의 `chat.completions.create()`를 호출한다.
- [ ] 응답 content 추출 helper를 SDK response shape에 맞게 추가한다.
- [ ] SDK 호출 중 발생하는 예외는 `None`으로 변환한다.
...
- [ ] LLM provider 설정 설명에 OpenAI provider는 공식 SDK를 사용한다고 짧게 추가한다.
- [ ] local provider는 OpenAI-compatible Chat Completions HTTP endpoint를 사용한다고 명시한다.
- [ ] `.env.example`은 기존 설정 키가 유효하면 변경하지 않는다.
...
- 위험: SDK client 생성 방식이 테스트 fake와 실제 SDK shape 사이에서 어긋날 수 있다.
  - 대응: protocol은 필요한 최소 surface만 정의하고, import smoke를 별도로 실행한다.

"""Undo helpers for Process Optimizer v2.

The helpers compare execution records with the current factory state and build
inverse commands only when the recorded before-state is reliable.
"""
...
def get_component_state(factory_state: Any, target_type: str, target_id: str) -> Optional[dict[str, Any]]:
    """Return the current machine or conveyor state from a factory snapshot."""
        ...
        for m in state_dict.get("machines", []):
            if m.get("id") == target_id:
                ...
                return {
                    ...
                    **{k: v for k, v in m.items() if k not in ["id", "type", "status", "enabled", "recipe_id"]}
                }
...
def check_undo_conflict(record: ExecutionRecord, factory_state: Any) -> bool:
    """Return True when the recorded after-state no longer matches the current factory state."""
    target_type = None
    target_id = None
    ...
    elif isinstance(record.before, dict) and "target" in record.before and record.before["target"]:
...
def build_inverse_command(record: ExecutionRecord) -> Optional[dict[str, Any]]:
    """Build a command that restores the recorded before-state when it is known."""
    ...
    if cmd_type == "set_recipe":
        ...
        if isinstance(before_state, dict):
            orig_recipe = before_state.get("recipe_id") or before_state.get("original_recipe_id") or orig_recipe
        return {
            "command": "set_recipe",

...
## 🆕 신규
- **`NS_SkyAvatar`** — **NamedSlot**, 좌하단 **기존 Sky 위치 그대로**(Sky 캐릭터+대사는 처음부터 좌하단 자리, 이동 아님). Sky **Live2D 자리**(플레이스홀더). 현재 C++ 계약 없음(자유). 추후 Live2D 콘텐츠 주입 슬롯.
...
| 좌하 | Sky 캐릭터 + 대사 (처음부터 이 자리) | `NS_SkyAvatar`(기존 Sky 위치) + `WBP_DialogueBalloon`(기존 자리 유지) |
| 중하 | 자동저장 | (자동저장 표시) |
| 우하 | 비움 | — |

> 좌하단 Sky 캐릭터+대사는 **처음부터 이 자리**(이동 아님). `NS_SkyAvatar` 슬롯도 기존 Sky 위치 그대로.
> `WBP_DialogueBalloon`은 **삭제 금지**(위치 미세조정만 가능).
...
- C++ 변경 0 → **`WBP_MainHUD.uasset` 단일 커밋** (+ DialogueBalloon 위치조정/NamedSlot 추가가 별 WBP 건드리면 해당 .uasset 포함)
